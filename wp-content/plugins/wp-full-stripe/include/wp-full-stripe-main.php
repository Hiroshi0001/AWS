...
class MM_WPFS {
	...
	function fullstripe_payment_form( $atts ) {
		...
		$payment_form = $this->database->get_payment_form_by_name( $form );
		...
		if ( ! $payment_form ) {
			...
			$style = $payment_form->formStyle;

			// tnagy load css and js only once per request
			$rendered_forms = self::get_rendered_forms()->render_later( WPFS_RenderedFormData::FORM_TYPE_PAYMENT );
	...
	function fullstripe_subscription_form( $atts ) {
		...
		if ( ! $subscription_form ) {
			...
			$style    = $subscription_form->formStyle;
			$allPlans = $this->get_plans();
			if ( count( $allPlans ) === 0 ) {
				$style = - 2;
			} else {
				$planIDs = json_decode( $subscription_form->plans );
				...
				// tnagy load css and js only once per request
				$rendered_forms = self::get_rendered_forms()->render_later( WPFS_RenderedFormData::FORM_TYPE_SUBSCRIPTION );
				if ( $rendered_forms->get_payments() + $rendered_forms->get_subscriptions() == 1 ) {
					$this->fullstripe_load_js();
				}
				if ( $rendered_forms->get_total() == 1 ) {
	...
	function fullstripe_checkout_form( $atts ) {
		...
		$checkout_form = $this->database->get_checkout_form_by_name( $form );
	...
	function fullstripe_thankyou_success( $attributes, $content = null ) {
		...
		if ( ! is_null( $transaction_data ) && $transaction_data instanceof MM_WPFS_TransactionData ) {
			...
			if ( $transaction_data instanceof MM_WPFS_SubscriptionTransactionData ) {
				...
				$currency_symbol = MM_WPFS::get_currency_symbol_for( $transaction_data->getPlanCurrency() );
				$search          = MM_WPFS_Utils::get_subscription_started_macros();
				$replace         = array(
					sprintf( '%s%0.2f', $currency_symbol, $transaction_data->getPlanSetupFee() / 100 ),
					$transaction_data->getPlanName(),
					sprintf( '%s%0.2f', $currency_symbol, $transaction_data->getPlanAmount() / 100 ),
					sprintf( '%s%0.2f', $currency_symbol, $transaction_data->getPlanAmount() / 100 ),
					$site_title,
				...
				$billing_address = $transaction_data->getBillingAddress();
				$currency_symbol = MM_WPFS::get_currency_symbol_for( $transaction_data->getCurrency() );
				$search          = MM_WPFS_Utils::get_payment_macros();
				$replace         = array(
					sprintf( '%s%0.2f', $currency_symbol, $transaction_data->getAmount() / 100 ),
					$site_title,
					$transaction_data->getCustomerName(),
					$transaction_data->getCustomerEmail(),
					$billing_address['line1'],
...
class MM_WPFS_Utils {
	 ...
	 * @return array
	 */
	public static function get_subscription_started_macros() {
		return array(
			"%SETUP_FEE%",
			"%PLAN_NAME%",
			"%PLAN_AMOUNT%",
	 ...
	 * @return array
	 */
	public static function get_subscription_finished_macros() {
		return array(
			"%PLAN_NAME%",
			"%PLAN_AMOUNT%",
			"%AMOUNT%",
