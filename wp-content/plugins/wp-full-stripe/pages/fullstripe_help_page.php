<p class="wrap">
	...
	<ol>
		...
		<li>Update the Full Stripe settings with your API keys and select the mode.<br/>(Test most is recommended
	...
	<h4>Payments</h4>
	<p>Now that the Stripe keys are set, you can create an inline payment form from the "Full Stripe -> Payments -> Payment Forms" page. A payment
	...
	<p>You can also create popup payment forms using the <a href="https://stripe.com/docs/checkout">Stripe Checkout</a>
		...
		the form is all controlled by Stripe and offers a fast and easy way to get started. <br/>
		Even better, the Stripe Checkout forms are responsive meaning you can have great looking payment forms on mobile
		devices as well. These forms act just like regular payment forms with regards to making
		charges and storing the information in your admin Payments table and your Stripe Dashboard.</p>
	<p>To show a popup form, add the following shortcode to any post or page:
		<code>[fullstripe_checkout form="formName"]</code> where "formName" equals the name you used to create the form.
	</p>
	<p>You can also accept Bitcoin payments with checkout forms. In order to use Bitcoin payments, make sure that the
		payment currency of your website is USD, and enable the "Use Bitcoin?" option on your checkout form.</p>
	<h4>Subscriptions</h4>
	<p>Similar to payments, you can sign customers up for recurring subscriptions using a subscription form. Before
		...
		Subscription Plans" page or from within your
		<a href="https://manage.stripe.com">Stripe Dashboard</a>. Both ways will create a subscription plan that will be
		listed on the Full Stripe Subscriptions page ready to use.
	</p>
	<p>When creating a subscription form you choose the name, title and the plans you wish to offer your customer using
		this form. To show a subscription form, add the following shortcode to any post or page:
	...
	<h4>Delete (local)</h4>
	<p>Some records are only deleted locally, this means the record is removed only from your local website database and
		not from your Stripe dashboard.</p>
	<h4>Styling</h4>
	<p>You can customize the appearance of the payment forms using the included CSS classes. Add CSS rules for <code>fullstripe-form-title</code>
		to style the form titles, <code>fullstripe-form-input</code> for the form inputs and
		<code>fullstripe-form-label</code> for the form labels.
		You can update the CSS in the Settings menu. There are defaults provided and it's recommended if you don't
		understand CSS you should leave these alone as it could break your forms.</p>
	<h4>Coupons</h4>
	...
	<p>All payment forms (payment inline, paymeny popup, and subscription inline) can send customized email notifications.<br/>
		...
		and WP Full Stripe will replace them with the relevant values at the point of successful payment. The tokens you
		can use are: <br/>
	<ul>
	...
	<ol>
		...
		<li>
			If you don't want to upload your .po file to second party, then run the <code>msgfmt</code> utility (part of the gettext distribution) to convert the .po file to an .mo
