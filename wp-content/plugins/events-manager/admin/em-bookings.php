...
function em_bookings_event(){
	...
	$localised_start_date = date_i18n('Y-m-d (D)', $EM_Event->start);
	$localised_end_date = date_i18n('Y-m-d (D)', $EM_Event->end);
	...
	<div class='wrap'>
		<?php if( is_admin() ): ?><h1><?php else: ?><h2><?php endif; ?>		
  			...
  			<a target='_blank' href='<?php echo EM_ADMIN_URL ."&amp;page=events-manager-bookings&action=bookings_report&event_id=".$EM_Event->event_id ?>' class="<?php echo $header_button_classes; ?>"><?php echo sprintf(_e('printable view','events-manager'))?></a>
		...
		$EM_Bookings_Table->status = 'pending';
		$EM_Bookings_Table->output();
  		?>
...
//		if( is_object($EM_Event) && !$EM_Event->can_manage('edit_events','edit_others_events') ){
		if( is_object($EM_Event) && !$EM_Event->can_manage('edit_others_events','edit_others_events') ){
			?>
			<div class="wrap"><h2><?php esc_html_e('Unauthorized Access','events-manager'); ?></h2><p><?php esc_html_e('You do not have the rights to manage this event.','events-manager'); ?></p></div>
			<?php
			return false;
