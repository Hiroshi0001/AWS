...
class SwpmAuth {
    ...
    private function check_constraints() {
        ...
        if (SwpmUtils::is_subscription_expired($this->userData)) {
            if ($this->userData->account_state == 'active') {
                $wpdb->update($wpdb->prefix . 'swpm_members_tbl', array('membership_level' => '5'), array('member_id' => $this->userData->member_id), array('%s'), array('%d'));
            }
    ...
    private function validate() {
        ...
        if (empty($user)) {
            $this->lastStatusMsg = SwpmUtils::_("ユーザー名が不正です。");
            return false;
        }
