...
foreach ($results as $value) {
	$corsename = $value->alias;
	if(strpos($value->subscr_id,'form: n1') !== false) {
		$corsename = "一日受け放題";
	}else if(strpos($value->subscr_id,'form: first_party') !== false){
		$corsename = "1st ANYBIRSARY";
	}else if(strpos($value->subscr_id,'form: single') !== false){
		$corsename = "any plus";
	}
